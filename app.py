...
def get_data():
    try:
        # Membaca file CSV
        df = pd.read_csv('velvet_apparel_ads_data.csv')
...
if df_selection.empty:
    st.warning("Data tidak ditemukan dengan filter yang Anda pilih.")
    st.stop()
...
st.markdown("Monitoring Kinerja Kampanye Digital Marketing (Q1 2025)")
st.markdown("---")
...
with col_right:
    st.subheader("📊 Efisiensi Platform")
...
# --- BAGIAN C: TABEL TRANSAKSI & PIE CHART ---
...
with col4:
    ...
    # 1. Ambil 10 data terbaru
    df_display = df_selection.sort_values(by=['Date', 'Time'], ascending=False).head(10).copy()
    
    # 2. Ubah format datetime menjadi date saja (menghilangkan 00:00:00)
...
# ==========================================
st.markdown("---")
st.caption("Dashboard dikembangkan oleh Kelompok Digital Marketing TI.22.A.AI.1")

csv = df_selection.to_csv(index=False).encode('utf-8')
st.download_button(
    label="📥 Download Laporan Lengkap (CSV)",
    data=csv,
    file_name='Laporan_Velvet_Apparel_Q1_2025.csv',
    mime='text/csv',
)